; ModuleID = '/local/mnt2/workspace2/mnadeem/llvm/llvm-test-suite/MultiSource/Applications/SPASS/kbo.c'
source_filename = "/local/mnt2/workspace2/mnadeem/llvm/llvm-test-suite/MultiSource/Applications/SPASS/kbo.c"
...
while.body:                                       ; preds = %if.end23, %while.end
...
while.end:                                        ; preds = %while.end.loopexit, %land.rhs.preheader
  ...
  %cmp.i221.not = icmp eq i32 %stack_POINTER.promoted290.lcssa, %0
  br i1 %cmp.i221.not, label %while.end56, label %while.body, !llvm.loop !8

while.end56.loopexit:                             ; preds = %while.body54
  store i32 %sub.i241358, ptr @stack_POINTER, align 4
  br label %while.end56

while.end56:                                      ; preds = %if.end47, %while.end, %while.end56.loopexit, %if.else, %if.then9
  %stack_POINTER.promoted303322 = phi i32 [ %0, %if.then9 ], [ %0, %if.else ], [ %0, %while.end56.loopexit ], [ %stack_POINTER.promoted303320, %if.end47 ], [ %stack_POINTER.promoted303319.lcssa, %while.end ]
  %Weight.1.lcssa = phi i32 [ 1, %if.then9 ], [ %10, %if.else ], [ %Weight.2, %while.end56.loopexit ], [ %Weight.2, %while.end ], [ %Weight.2, %if.end47 ]
...
if.then136:                                       ; preds = %if.end128
  ...
  %or.cond = select i1 %tobool137.not, i1 true, i1 %cmp115.not
...
for.body.preheader.i:                             ; preds = %entry
  %0 = zext i32 %spec.select.i to i64
...
for.inc:                                          ; preds = %land.rhs
  ...
  br i1 %cmp22, label %land.rhs, label %cleanup67.thread, !llvm.loop !17
...
while.body:                                       ; preds = %land.rhs37
  ...
  br i1 %cmp.i181.not, label %cleanup67.thread, label %land.rhs37, !llvm.loop !18
...
for.body:                                         ; preds = %if.else, %for.body
  ...
  br i1 %cmp.i.not, label %if.end, label %for.body, !llvm.loop !19
