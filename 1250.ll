; ModuleID = '/local/mnt2/workspace2/mnadeem/llvm/llvm-test-suite/SingleSource/Regression/C/gcc-c-torture/execute/20050502-1.c'
source_filename = "/local/mnt2/workspace2/mnadeem/llvm/llvm-test-suite/SingleSource/Regression/C/gcc-c-torture/execute/20050502-1.c"
...
entry:
  %call = tail call i32 @bar(ptr noundef %w)
  %c.036 = trunc i32 %call to i8
  store i8 %c.036, ptr %x, align 1, !tbaa !9
  %call237 = tail call i32 @bar(ptr noundef %w)
  %sext38 = shl i32 %call237, 24
  %cmp40 = icmp eq i32 %sext38, 654311424
  %or.cond41 = select i1 %y, i1 %cmp40, i1 false
  %cmp1142 = icmp eq i32 %sext38, 570425344
  %or.cond2643 = select i1 %z, i1 %cmp1142, i1 false
  %or.cond3544 = select i1 %or.cond41, i1 true, i1 %or.cond2643
  br i1 %or.cond3544, label %while.end, label %if.end14.lr.ph
...
if.end14.preheader:                               ; preds = %if.end14.lr.ph
  %conv54757 = ashr exact i32 %sext38, 24
  %call2058 = tail call i32 @baz(i32 noundef %conv54757)
  %tobool21.not59 = icmp eq i32 %call2058, 0
  br i1 %tobool21.not59, label %while.end, label %if.end23

if.end14.us:                                      ; preds = %if.end14.lr.ph, %if.end14.us
  %indvars.iv51 = phi i64 [ %indvars.iv.next52, %if.end14.us ], [ 1, %if.end14.lr.ph ]
  %call246.us = phi i32 [ %call2.us, %if.end14.us ], [ %call237, %if.end14.lr.ph ]
  %c.0.us = trunc i32 %call246.us to i8
  %indvars.iv.next52 = add nuw i64 %indvars.iv51, 1
  %arrayidx.us = getelementptr inbounds i8, ptr %x, i64 %indvars.iv51
  store i8 %c.0.us, ptr %arrayidx.us, align 1, !tbaa !9
  %call2.us = tail call i32 @bar(ptr noundef %w)
  %sext.us = shl i32 %call2.us, 24
  %cmp.us = icmp eq i32 %sext.us, 654311424
  %or.cond.us = select i1 %y, i1 %cmp.us, i1 false
  %cmp11.us = icmp eq i32 %sext.us, 570425344
...
if.end23:                                         ; preds = %if.end14.preheader, %if.end23
  %call24661 = phi i32 [ %call2, %if.end23 ], [ %call237, %if.end14.preheader ]
  %indvars.iv60 = phi i64 [ %indvars.iv.next, %if.end23 ], [ 1, %if.end14.preheader ]
  %c.0 = trunc i32 %call24661 to i8
  %indvars.iv.next = add nuw i64 %indvars.iv60, 1
  %arrayidx = getelementptr inbounds i8, ptr %x, i64 %indvars.iv60
  store i8 %c.0, ptr %arrayidx, align 1, !tbaa !9
  %call2 = tail call i32 @bar(ptr noundef %w)
  %sext = shl i32 %call2, 24
  %conv547 = ashr exact i32 %sext, 24
  %call20 = tail call i32 @baz(i32 noundef %conv547)
  %tobool21.not = icmp eq i32 %call20, 0
  br i1 %tobool21.not, label %while.end, label %if.end23

while.end:                                        ; preds = %if.end23, %if.end14.us, %if.end14.preheader, %entry
  %inc.lcssa = phi i64 [ 1, %entry ], [ 1, %if.end14.preheader ], [ %indvars.iv.next52, %if.end14.us ], [ %indvars.iv.next, %if.end23 ]
